...
const ImportantNotePR = () => {
  ...
  const fetchNotes = async () => {
    try {
      const res = await fetch(`/api/important-notes/saved/${page}`);
 ...
 const handleSubmit = async () => {
  if (!content) return;
  try {
    const res = await fetch("/api/important-notes/save", {
      method: "POST",
  ...
  const handleDelete = async (noteId) => {
    ...
    try {
      const res = await fetch(`/api/important-notes/delete/${noteId}`, {
        method: "DELETE",
      });
      if (res.ok) {
              ...
              <ul style={styles.notesList}>
                {notes.map((note) => (
                  <li key={note._id} style={styles.noteItem}>
                    <strong>{note.author?.trim() || "Anonymous"}:</strong> {note.content}
